...
export function ContactInfoCards() {
  return (
    ...
    >
      {/* İletişim Kanalları — konsolide kart. Ofis adresleri kaldırıldı; tek kaynak alttaki OfficeShowcase + footer. */}
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm text-(--color-text-muted) font-light">
                  İletişim Kanalları
                </span>
              </div>
            </div>

            {/* Çağrı Merkezi — vurgulu birincil kanal */}
            <a
              href="tel:4442673"
              className="flex items-center gap-4 p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-colors mb-4"
            >
              <div className="h-11 w-11 rounded-xl bg-blue-500/15 border border-blue-500/20 flex items-center justify-center text-(--color-accent-blue-light) shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {contactData.contactInfo.map((channel) => {
                const isFax = channel.title === "Fax";
                const Icon = isFax ? Printer : Phone;

                const content = (
                  <>
                    <Icon className="w-4 h-4 text-(--color-text-muted) shrink-0" />
                    <div className="flex flex-col min-w-0">
                      <span className="text-xs font-medium text-(--color-text-muted) mb-0.5">{channel.title}</span>
                      <span className="text-(--color-brand-accent) font-medium text-sm tracking-tight truncate">
                        {channel.value}
                      </span>
                    </div>
                  </>
                ...
                return isFax ? (
                  <div
                    key={channel.title}
                    className="flex items-center gap-3 p-3 rounded-2xl bg-white/2 border border-white/5"
                  >
                    {content}
                  </div>
                ) : (
                  <a
                    ...
                    className="flex items-center gap-3 p-3 rounded-2xl bg-white/2 border border-white/5 hover:bg-white/4 hover:border-(--color-brand-accent)/30 transition-all"
                  >
                    {content}
                  </a>
                );
