...
export interface AddonCostPageData {
  hero: { badge: string; titleLead: string; titleAccent: string; lead: string; support: string };
  ozet: {
    overline: string;
    title: string;
    description: string;
    items: { no: string; label: string; hint: string }[];
  };
  yanilsama: {
    overline: string;
    title: string;
    intro: string;
    items: { myth: string; fact: string; text: string }[];
  };
  sutunlar: {
    overline: string;
    title: string;
  ...
  efor: { overline: string; title: string; intro: string; items: LabelledItem[] };
  bakim: { overline: string; title: string; items: LabelledItem[] };
  karsilastirma: {
    overline: string;
    title: string;
    columns: { parametre: string; canli: string; canliEn: string; test: string; testEn: string };
    rows: { parametre: string; canli: string; test: string }[];
  };
  sektor: { overline: string; title: string; intro: string; items: LabelledItem[] };
  faq: { overline: string; title: string; items: { question: string; answer: string }[] };
  kapanis: {
    overline: string;
    title: string;
    text: string;
  ...
  ilgili: { overline: string; items: { label: string; desc: string; href: string }[] };
  /** Sayfa sonundaki kapsam/teklif notu — her add-on kendi adiyla yazar. */
  kapsamNotu: string;
}
...
/**
 * Altyapi ve sistem add-on'larinin maliyet yapisini anlatan sayfa sablonu.
 * Uc kalem (lisans / kurulum / bakim) ayni kurgu ile sunulur; icerik
 * src/data/addon-sayfalari/<slug>.json dosyalarindan gelir.
 */
export default function AddonCostPage({
  data,
  badgeIcon: BadgeIcon,
  glowColor,
}: {
  data: AddonCostPageData;
    ...
    <div className="relative min-h-screen bg-(--color-surface-base) overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-12 pt-32">
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-(--color-brand-primary)/40 bg-(--color-brand-primary)/8 mb-10">
          <BadgeIcon className="w-3.5 h-3.5 text-(--color-brand-accent)" aria-hidden="true" />
